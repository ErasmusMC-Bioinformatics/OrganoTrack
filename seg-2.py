...
img_eroded = cv.dilate(cv.erode(img_blur, struct_element),struct_element)
...
struct_element_shape = cv.MORPH_ELLIPSE
struct_element = cv.getStructuringElement(struct_element_shape, (2 * struct_element_size + 1, 2 * struct_element_size + 1))
img_dilated = cv.erode(cv.dilate(img_eroded, struct_element), struct_element)
...
for j in range(20, 251, 10):
    adaptiveIter = cv.adaptiveThreshold(img_dilated, 255, cv.ADAPTIVE_THRESH_MEAN_C, cv.THRESH_BINARY, j-1, 2)
