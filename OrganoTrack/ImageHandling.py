...
def ConvertImagesToPILImageStacks(images: np.ndarray, originalImages: List[Image.Image],
                                  resize=True):
    stacks = ConvertImagesToStacks(images, originalImages)  # original Images: List[Image.Image]
    # stacks: a List of 3D numpy arrays

    if resize:
        return [
            [Image.fromarray(d).resize(o.size, resample=Image.Resampling.NEAREST) for d in stack] for o, stack in zip(originalImages, stacks)]
    else:
        return [[Image.fromarray(d) for d in stack] for stack in stacks]
        # Image.fromarray(d) gets one 2D array from a 3D array
